...
- Package name: `prowlqa` (global install via npm).
- Node.js 20+ / TypeScript.
- Chromium only for MVP.
- Config: `.prowlqa/config.yml`; hunts: `.prowlqa/hunts/*.yml`; artifacts: `.prowlqa/runs/<timestamp>/`.
- `prowlqa login` captures `storageState` and saves to `.prowlqa/auth-state.json`.
...
- `.env` location: same directory as resolved `.prowlqa/config.yml`.
- `prowlqa init` refuses if `.prowlqa/` exists unless `--force`.
