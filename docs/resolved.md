...
**Description**: `prowlqa ci` now groups failed hunts that share a common cause so one root cause surfaces instead of N independent failures. New `src/runner/clustering.ts` (`clusterFailures`) groups `BugFailure[]` by step type + selector + normalized error (reusing `normalizeError` from the bug-logging fingerprinter), returning `FailureCluster[]` sorted by size then cause. `suite.ts` runs it over the suite's failures (via the existing `extractFailures`) and surfaces only multi-hunt clusters as `CiResult.clusters` (omitted when none) — in a "Failure clusters" section of the CI summary and as a `clusters` array in `ci-result.json` / `prowlqa ci --json`. Exported `clusterFailures`/`FailureCluster`/`CiFailureCluster` from the library; documented in README + CHANGELOG. Covered by 7 unit + 2 suite-integration tests; 585 tests pass, build + lint clean. Scope note: clustered by step type/selector/normalized error (the dimensions `extractFailures` provides); the AC's "same URL" dimension was deferred since `BugFailure` doesn't carry the target URL — a clean follow-up if wanted.
...
**Description**: When a hunt hits a failing request (HTTP status >= 400), ProwlQA now reads the response's `traceparent` header, extracts the W3C trace id, and surfaces it in `result.json` (`traceCorrelations`) and a "Trace Correlations" section of `summary.md` — so a developer can pivot from a hunt failure to the matching distributed trace in their own APM (Datadog, Grafana/Tempo, Jaeger, etc.). New `src/runner/tracing.ts` (`parseTraceId` handles W3C `traceparent` and non-standard headers; `captureTraceCorrelation` no-ops when the header is absent/empty). The header is configurable via `tracing.header` in `config.yml` (default `traceparent`), Zod-validated and strict. `RunResult.traceCorrelations` is omitted entirely when empty so clean runs stay tidy. Explicitly a correlation bridge only — ProwlQA does not generate or propagate its own spans. Covered by `test/tracing.test.ts`, summary-rendering tests, and config-schema tests; documented in the README.
...
**Description**: Lets one `prowlqa mcp` server drive multiple repos instead of one server per project. Added `src/mcp/projects.ts`: a zod-validated YAML registry mapping project names to repo roots (`projects: { coupe: { root: /path, configPath?: ... } }`), resolved in priority order from `prowlqa mcp --projects <path>`, the `PROWLQA_PROJECTS` env var, or `~/.prowlqa/projects.yml` (no registry → current-directory mode). Added a `list_projects` tool and an optional `project` argument on `list_hunts`/`run_suite`/`run_hunt` that resolves the named project to its `.prowlqa/config.yml` and threads it through `loadConfig`/`runSuite`/`runHunt` (so bug-logging lands in that repo's `docs/backlog.md`); omitting `project` preserves the existing cwd behavior. Was the deferred follow-up to the Agent QA / MCP Server epic — tracked here directly (no backlog entry) at the user's request. Covered by registry unit tests and in-memory client↔server integration tests (list_projects, project targeting, unknown-project and no-registry errors); verified with a real stdio smoke test against the built binary.

## ~~P5-001: MCP Server (`prowlqa mcp`)~~
**Resolved**: 2026-05-25 (branch mcp-server, commit 9ebe090)
**Description**: Added `prowlqa mcp`, a stdio MCP server (`@modelcontextprotocol/sdk` v1.29, `McpServer.registerTool`) that exposes ProwlQA to AI agents as three named tools: `list_hunts` (hunts in run order), `run_suite` (runs all hunts via `runSuite()` then logs failures via `updateBacklogFromSuite()`, returning pass/fail/skip counts plus the created/regression `QA-NNN` ticket ids), and `run_hunt` (single hunt → full `RunResult`). Tool logic lives in `src/mcp/tools.ts`, server wiring in `src/mcp/server.ts`, and a `prowlqa mcp` subcommand registered in `program.ts`. Operates on the current repo's `.prowlqa/` (cwd), reuses the CLI core so the tool surface always matches the step schema, and existing guardrails apply unchanged. Covered by unit tests (tool functions) and an integration round-trip test (in-memory client↔server), plus a real stdio handshake smoke test. Completes the Agent QA / MCP Server epic (P5-008 → P5-009 → P5-001).
